...
const AdminExamReview = () => {
  ...
  if (loading) {
    return (
      <div className="space-y-6">
        <div className="h-10 w-1/3 bg-stone-mid rounded animate-pulse" />
      ...
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full max-w-xs">
          <button
            ...
            className="flex-1 flex items-center justify-center gap-2 rounded-md border border-stone-deep bg-white px-5 py-3 font-ui text-[13px] font-bold text-ink-3 hover:bg-stone-wash transition-all cursor-pointer"
          >
          ...
          {!is404 && (
            <button
              ...
              className="flex-1 flex items-center justify-center gap-2 rounded-md bg-navy px-5 py-3 font-ui text-[13px] font-bold text-[#F4F0E4] hover:bg-navy-mid transition-all cursor-pointer"
            >
  ...
  const { examTitle, studentName, regimentalNumber, score, correct, incorrect, skipped, total, submittedAt } = data;

  return (
    <div className="space-y-6 pb-12">
      <PageHeader
        title={examTitle}
        subtitle={`Cadet: ${studentName || 'Unknown'} ${regimentalNumber ? `(${regimentalNumber})` : ''}`}
        action={
          <button
            onClick={() => goBack('/admin/results')}
            className="h-[36px] px-[18px] rounded-md font-ui text-[13px] font-medium flex items-center gap-2 transition-all bg-white border border-stone-deep text-ink-2 hover:bg-stone"
          >
