...
import { Download, Search, Edit3, XCircle, ShieldAlert, CheckCircle2 } from 'lucide-react';
...
export default function ResultsBoard() {
  ...
  if (loading) return <div className="p-10 text-ink-4 font-mono text-[13px]">Loading exam results...</div>;
  ...
  return (
    <div>
      ...
      <div className="bg-white border border-stone-deep p-5 rounded-md shadow-sm mb-8 grid grid-cols-1 sm:grid-cols-3 gap-5">
        ...
        <div>
          <label className="block font-mono text-[10px] tracking-[0.1em] uppercase text-ink-3 mb-1.5">Filter by Exam</label>
          <select 
            className="w-full h-[38px] px-3 border border-stone-deep rounded-md font-ui text-[14px] text-ink bg-white outline-none focus:border-navy-soft focus:ring-[3px] focus:ring-navy-wash transition-all" 
            value={filters.exam}
        ...
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-[#0E1929]/40 backdrop-blur-sm">
          <div className="bg-[#FDFCF8] border border-stone-deep rounded-2xl w-full max-w-[450px] overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.15)]">
            ...
            <form onSubmit={handleOverride} className="p-6">
              ...
              <div className="mb-5">
                <label className="block font-mono text-[10px] tracking-[0.1em] uppercase text-ink-3 mb-1.5">New Score (%)</label>
                <input 
                  type="number" 
                  className="w-full h-[38px] px-3 border border-stone-deep rounded-md font-ui text-[14px] text-ink bg-white outline-none focus:border-navy-soft focus:ring-[3px] focus:ring-navy-wash transition-all" 
                  min="0" 
              ...
              <div className="mb-5">
                ...
                <textarea 
                  ...
                  placeholder="e.g. Technical error during submission, re-evaluation request..."
                  required
                  value={overrideForm.reason}
                  onChange={e => setOverrideForm({...overrideForm, reason: e.target.value})}
                />
                <div className="text-[10px] text-ink-4 mt-1 font-ui">* This action will be recorded in the audit logs.</div>
              </div>
