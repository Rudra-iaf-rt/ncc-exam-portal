...
export const useProctoring = ({ onSecurityBreach, maxWarnings = 3 }) => {
  ...
  // attemptId injected from ExamAttempt via ref so we don't need it as a prop
  ...
  const recordViolation = useCallback(async (type) => {
    ...
    if (attemptIdRef.current) {
      try {
        const { data } = await examApi.saveViolation(attemptIdRef.current, { type });
  ...
  return {
    ...
    requestFullscreen,
    requestScreenShare,
    setAttemptId, // Expose so ExamAttempt can inject attemptId after startExam resolves
    screenStream,
  };
};
